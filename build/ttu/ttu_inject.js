(() => {
  ...
  var TtuStorage = class extends MediaStorage {
    ...
    async processText(chars_read, date) {
      ...
      await this.setDailyStats(date, stats);
      await this.instance_storage.addToDates(date);
      await this.instance_storage.addToDate(date);
    }
    async setDailyStats(date, values) {
      const uuid_date_key = JSON.stringify([this.uuid, date]);
      let daily_stats_entry = await browser4.storage.local.get(uuid_date_key);
      daily_stats_entry[uuid_date_key] = values;
      if (date == dateNowString()) {
        this.today_stats = daily_stats_entry[uuid_date_key];
      }
      await browser4.storage.local.set(daily_stats_entry);
    }
  };
  ...
  function getCharCount() {
    let charCount = document.getElementsByClassName("writing-horizontal-tb")[1]["innerText"];
  ...
  async function initialSetup(char_current, book_title) {
    ...
    new ttu_default({
      target: document.body.querySelector(".book-content"),
      props: {
        ttu_storage
      }
    });
