...
export async function exportStats() {
  ...
  const blob = csv_blob(unparse(data!), { type: "text/csv" });
...
export async function exportLines() {
  ...
  const detail_entries = await browser.storage.local.get(
    Object.values(media["media"]),
  );
  const data = await Promise.all(
    Object.entries(detail_entries).map(getInstanceData),
  );

  const blob = csv_blob(unparse(data.flat()), {
