...
function getPage() {
  const [current_page, total_pages] = document
    .getElementById("pageIdxDisplay")!
    .innerText.split("/");

  return [Number.parseInt(current_page) - 1, Number.parseInt(total_pages)];
}

const getGivenID = (series: string, volume: string) =>
  JSON.stringify([series, volume]);

async function setup() {
  // Ensure single page mode is on
  (document.getElementById("menuDoublePageView") as HTMLInputElement).checked =
    true;
  ...
  await mokuro_storage.instance_storage?.updateDetails({
...
const observer = new MutationObserver(async (_) => {
  ...
  const get_page =
    current_page > mokuro_storage.details?.last_page_read!
      ? current_page - 1
      : current_page;
  const lines = Array.from(
    document.getElementById(`page${get_page}`)!.firstChild!.childNodes,
  ).map((element) =>
    Array.from(element.childNodes).reduce(
      (so_far, node) => `${so_far}${node.textContent}`,
      "",
    ),
  );
...
observer.observe(document.getElementById("pageIdxDisplay")!, {
  childList: true,
  subtree: true,
});
