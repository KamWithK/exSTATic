...
import type { InstanceDetails, InstanceStorage, Stat } from "../storage/instance_storage"
...
export class MokuroStorage extends MediaStorage<MokuroDetails> {
    static async build(live_stat_update=false) {
        ...
        return new MokuroStorage(media_storage.type_storage, media_storage.instance_storage, live_stat_update)
    }
    ...
    async processPage(page_num: number, lines: string[], date: string) {
        let stats: Partial<MokuroStat> = {
            ...
            lines_read: lines.reduce((total, line) => total + lineSplitCount(line), 0),
