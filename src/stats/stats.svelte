<script lang="ts">
    ...
    data = client_groups.map(([, v]) => ({
        "uuid": v[0].type === "mokuro" ? JSON.parse(v[0].given_identifier)[0] : v[0].uuid,
        "name": v[0].type === "mokuro" && v[0].name === v[0].given_identifier ? JSON.parse(v[0].given_identifier)[0] : v[0].name,
