...
import type { InstanceDetails, InstanceStorage } from "../storage/instance_storage"
...
export class TTUStorage extends MediaStorage<TTUDetails> {
    static async build(live_stat_update=false) {
        ...
        return new TTUStorage(media_storage.type_storage, media_storage.instance_storage, live_stat_update)
    }
    ...
    async processText(chars_read: number, date: string) {
        const stats = { "chars_read": chars_read - (this.details?.last_char_count ?? 0) }
