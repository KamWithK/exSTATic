...
async function setup() {
  ...
  document.body.insertBefore(
    svelte_div,
    document.querySelector("div.h-full.w-full"),
  );
...
const onUpdate = async (event: CustomEvent) => {
  ...
  if (active) await ttu_storage.processText(char_count, dateNowString());
  else await ttu_storage.pauseChange(char_count);
};
...
document.addEventListener(
  "ttsu:page.change",
  (event: CustomEvent) =>
    (promise_chain = promise_chain.then(() => onUpdate(event))),
);
